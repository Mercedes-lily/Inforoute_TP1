...
const GraphQLPage = () => {
  ...
  const [regroupementFilter, setRegroupementFilter] = useState("tous");
  const [defavoriseeFilter, setDefavoriseeFilter] = useState("tous");
  ...
  const chartData = useMemo(() => {
    ...
    etablissements.forEach((ecole: any) => {
      // Notez le "nomCourt" au lieu de "nom_court"
      const name = ecole.regroupement?.nomCourt || "Inconnu";
  ...
  const filteredItems = etablissements.filter((ecole: any) => {
    const matchesSearch =
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-primary">
            Liste des Établissements (GraphQL)
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading && (
                    ...
                    {filteredItems.length > 0 ? (
                      filteredItems.map((ecole: any) => (
                        <TableRow key={ecole.id} className="hover:bg-muted/30">
                          ...
                          <TableCell>{ecole.type}</TableCell>
                          ...
                          <TableCell>{ecole.regroupement?.nomCourt}</TableCell>
                        </TableRow>
                      ))
                    ) : (
                      <TableRow>
